...
def read_voc_images(root="../VOC2012",
                    is_train=True, max_num=None):
    txt_fname = '%s/ImageSets/Segmentation/%s' % (# 第一个%s用root值填入，第二个用if的结果填入
...
colormap2label = torch.zeros(256 ** 3, dtype=torch.uint8)# 三通道有256*256*256种情况
...
def voc_label_indices(colormap, colormap2label):
    ...
    idx = ((colormap[:, :, 0] * 256 + colormap[:, :, 1]) * 256# 三色表示，就是把一个二维数组里的值换成一个三维向量，成为三维数组
           + colormap[:, :, 2])
    return colormap2label[idx]

y = voc_label_indices(train_labels[0], colormap2label)
print(y[105:115, 130:140], VOC_CLASSES[1])
#使用图像增广里的随机裁剪，并对输入图像和标签裁剪相同区域
# 本函数已保存在d2lzh_pytorch中方便以后使用
